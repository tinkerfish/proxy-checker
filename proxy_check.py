...
def get_args(argv=None):
    parser = argparse.ArgumentParser(prog='ProxyCheck',
                                     usage='%(prog)s [-h] -i PROXY_FILE [-o OUT_FILE] -t THREADS',
                                     description='Test list of proxies for reachability and function')
    parser.add_argument('-i', '--proxy_file',
                        required=True,
                        help='Proxy File Input')
...
def verify_proxy(proxy_list, thread_number):
    ...
    for item in proxy_list:
        try:
            ...
            print(f'[Thread: {thread_number}] Proxy Works: {True if ip == item.split(":")[0] else False}')
